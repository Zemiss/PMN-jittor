...
class DeepUnet(nn.Module):
    ...
    def execute(self, x, noise_map=None):
        shape= x.size()
        ...
        if self.training:
            ...
            if self.res:
                x2 = nn.avg_pool2d(x, 2)
                x4 = nn.avg_pool2d(x2, 2)
                x8 = nn.avg_pool2d(x4, 2)
                out += x
                out2 += x2
                out4 += x4
                out8 += x8
