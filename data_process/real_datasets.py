...
class RealBase_Dataset(Dataset):
    ...
    def eval_crop(self, data, base=64):
        ...
        nh = self.h // l + 1
        nw = self.w // l + 1
        data = jt.nn.pad(data, (d, d, d, d), mode='reflect')
        croped_data = jt.empty((nh, nw, self.c, crop_size, crop_size),
                    dtype=data.dtype)
        # 分块crop主体区域
        for i in range(nh-1):
            for j in range(nw-1):
    ...
    def eval_merge(self, croped_data, base=64):
        crop_size = self.args["patch_size"]
        data = jt.empty((1, self.c, self.h, self.w), dtype=croped_data.dtype)
        # crop setting
        d = base//2
        l = crop_size - base
        nh = self.h // l + 1
...
class TestDataset(RealBase_Dataset):
    ...
    def default_args(self):
        self.args = {}
        self.args['data_dir'] = 'D:/data/SID/Sony/long'
