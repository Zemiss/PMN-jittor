...
def bayer2raw(packed_raw, wp=16383, bl=512):
    if isinstance(packed_raw, jt.Var):
        packed_raw = packed_raw.detach()
        packed_raw = packed_raw[0].numpy().astype(np.float32)
...
def postprocess_bayer(rawpath, img4c, white_point=1023):
    if isinstance(img4c, jt.Var):
        img4c = img4c.detach()
        img4c = img4c[0].numpy().astype(np.float32)
...
def gamma_compression(images, gamma=2.2):
    ...
    outs = jt.clamp(images, min_v=1e-8) ** (1 / gamma)
    # outs = (1 + gamma[0]) * np.power(images, 1.0/gamma[1]) - gamma[0] + gamma[2]*images
    outs = jt.clamp((outs*255).int(), min_v=0, max_v=255).float() / 255
    return outs
...
def process(bayer_images, wbs, cam2rgbs, gamma=2.2):
    ...
    # Binning
    bayer_images = jt.clamp(bayer_images, min_v=0.0, max_v=1.0)
    images = raw2LRGB(bayer_images)
    # Color correction.
    images = apply_ccms(images, cam2rgbs)
    # Gamma compression.
    images = jt.clamp(images, min_v=0.0, max_v=1.0)
...
def raw2rgb_v2(packed_raw, wb, ccm):
    if isinstance(packed_raw, jt.Var):
        packed_raw = packed_raw.detach().float()
    else:
        packed_raw = jt.array(packed_raw).float()
    wb = jt.array(wb).float()
...
def get_aug_param_torch(data, b=8, command='augv1.2', numpy=False):
    ...
    # 后续工作会有策略更新，敬请期待。
    aug_r, aug_g, aug_b = jt.zeros(b), jt.zeros(b), jt.zeros(b)
    ...
    if np.random.randint(4):
        ...
        if 'augv1.1' in command:
            if ratioE > 2:
                u = ratioE / 2 - 1
                aug_g = jt.clamp(jt.randn(b) * r, min_v=-3*r, max_v=3*r) + u
                aug_g = jt.clamp(aug_g, min_v=0, max_v=4*u)
                aug_r = (aug_g+1) * (1 + jt.randn(b) * r) - 1
                aug_b = (aug_g+1) * (1 + jt.randn(b) * r) - 1
                aug_r = jt.clamp(aug_r, min_v=0, max_v=4*u)
                aug_b = jt.clamp(aug_b, min_v=0, max_v=4*u)
            ...
            if ratioE > 2:
                ...
                if np.random.randint(4) < 1:
                    u = 0.25
                    aug_g = jt.randn(b) * r + u
                    aug_g = jt.clamp(aug_g, min_v=0, max_v=4*u)
                    aug_r = (aug_g+1) * (1 + jt.randn(b) * r) - 1
                    aug_b = (aug_g+1) * (1 + jt.randn(b) * r) - 1
                    aug_r = jt.clamp(aug_g, min_v=0, max_v=4*u)
                    aug_b = jt.clamp(aug_g, min_v=0, max_v=4*u)
...
def raw_wb_aug_torch(noisy, gt, aug_wb=None, camera_type='IMX686', ratio=1, ori=True, iso=None, ratiofix=False):
    ...
    if aug_wb is not None:
        ...
        daug = -np.minimum(np.min(aug_wb), 0)
        daug = jt.array(daug)
        aug_wb = jt.array(aug_wb)
        dy = gt * aug_wb.reshape(-1,1,1)    # 我考虑过这里dy和dn要不要量化一下，感觉不量化对多样性更友好
        if daug == 0:
            # 只有增益的话很好处理，叠加泊松分布就行
            # Jittor doesn't have built-in Poisson, use numpy approximation
            dn = jt.array(np.random.poisson((dy/p['K']).numpy()).astype(np.float32)) * p['K']
        else:
            # 存在减益的话就很麻烦，需要考虑read noise并且补齐分布
            scale = 1 - daug
            # 要保证dyn是非负的
            ...
            noisy *= scale
            # 补齐单个照片的读噪声
            dn_read = jt.randn(*gt.shape) * p['sigGs'] * jt.sqrt(1-scale**2)
            # 补齐由于除法导致的分布变化，恢复shot noise应有的分布
            scale_sigma = scale - scale**2
            dn_shot = jt.array(np.random.poisson((scale_sigma * gt/p['K']).numpy()).astype(np.float32)) * p['K'] - gt * scale_sigma
            # 叠加泊松分布
            dn_aug = jt.array(np.random.poisson((dyn/p['K']).numpy()).astype(np.float32)) * p['K']
            dn = dn_read + dn_shot + dn_aug
        if ratiofix:
            ratio = ratio / (1 + daug)
        gt = jt.clamp((gt + dy)*ratio, min_v=0, max_v=(p['wp'] - p['bl']))
        noisy = jt.clamp(noisy + dn, min_v=-p['bl'], max_v=(p['wp'] - p['bl']))
        gt /= (p['wp'] - p['bl'])
        noisy /= (p['wp'] - p['bl'])
...
def generate_noisy_torch(y, camera_type=None,  noise_code='p', param=None, MultiFrameMean=1, ori=False, clip=False):
    ...
    if use_P:   # 使用泊松噪声作为shot noisy
        # Jittor doesn't have built-in Poisson, use numpy approximation
        noisy_shot = jt.array(np.random.poisson((MFM*y/p['K']).numpy()).astype(np.float32)) * p['K'] / MFM
    else:   # 不考虑shot noisy
        noisy_shot = (y + jt.randn(*y.shape) * jt.sqrt(jt.maximum(y/p['K'], 1e-10))) * p['K'] / MFM
    if not use_black:
        if use_TL:   # 使用TL噪声作为read noisy
            raise NotImplementedError
            # noisy_read = stats.tukeylambda.rvs(p['lam'], scale=p['sigTL'], size=y.shape).astype(np.float32)
        else:   # 使用高斯噪声作为read noisy
            noisy_read = jt.randn(*y.shape) * (p['sigGs']/MFM)
    else:
        noisy_read = 0
    # 使用行噪声
    noisy_row = jt.randn(y.shape[-3], y.shape[-2], 1) * p['sigR'] / MFM if use_R else 0
    noisy_q = (jt.rand(*y.shape) - 0.5) * p['q'] * (p['wp'] - p['bl']) if use_Q else 0
    noisy_bias = p['bias'] if use_D else 0

    # 归一化回[0, 1]
    z = (noisy_shot + noisy_read + noisy_row + noisy_q + noisy_bias) / (p['wp'] - p['bl'])
    # 模拟实际情况
    z = jt.clamp(z, min_v=-p['bl']/p['wp'], max_v=1) if clip is False else jt.clamp(z, min_v=0, max_v=1)
    # ori_brightness
    if ori is False:
        z = z * p['ratio']
...
class ELDEvalDataset(jt.dataset.Dataset):
    def __init__(self, basedir, camera_suffix=('NikonD850','.nef'), scenes=None, img_ids=None):
        ...
        # self.input_dict = {}
        # self.target_dict = {}
        self.set_attrs(total_len=len(self.scenes) * len(self.img_ids))
