...
class SID_Trainer(Base_Trainer):
    def __init__(self):
        ...
        self.optimizer = optim.Adam(self.net.parameters(), lr=self.hyper['learning_rate'])
        ...
        log(f"Using Jittor with CUDA: {jt.has_cuda}, GPU Count: {jt.world_size}", log=self.logfile, notime=True)
        if 'gpu_preprocess' in self.dst and self.dst['gpu_preprocess']:
            log("Using Jittor's GPU Preprocess...")
            self.use_gpu = True
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.hyper['last_epoch']+1, self.hyper['stop_epoch']+1):
            ...
            with tqdm(total=len(self.dataloader_train), disable=jt.rank>0) as t:                
                for k, data in enumerate(self.dataloader_train):
                    ...
                    # Jittor: 使用 optimizer.step(loss) 自动处理 zero_grad 和 backward
            ...
            # 存储模型
            if epoch % self.hyper['save_freq'] == 0 and jt.rank == 0:
                model_dict = self.net.state_dict()
                epoch_id = epoch // self.hyper['plot_freq'] * self.hyper['plot_freq']
                save_path = os.path.join(self.model_dir, '%s_e%04d.pkl'% (self.model_name, epoch_id))
                jt.save(model_dict, save_path)
            ...
            if self.save_plot and jt.rank == 0:
            ...
            if (self.hyper['last_epoch']+epoch) % self.hyper['plot_freq'] == 0:
                ...
                if jt.rank == 0:
                    model_dict = self.net.state_dict()
                    jt.save(model_dict, f'./checkpoints/{self.model_name}_last_model.pth')
    ...
    def eval(self, epoch=-1):
        ...
        # record every metric
        metrics = {}
        metrics_path = f'./metrics/{self.model_name}_metrics_rank{jt.rank}.pkl'
        ...
        with tqdm(total=len(self.dataloader_eval), disable=jt.rank>0) as t:
            for k, data in enumerate(self.dataloader_eval):
                ...
                name = data['name'][0]
                # 修复 Windows 路径问题：只保留文件名，去掉路径中的冒号
                ...
                with jt.no_grad():
                    ...
                    # croped_imgs_dn = jt.contrib.concat(croped_imgs_dn)
                    ...
                    if imgs_lr.shape[-1] % 16 != 0:
                        p2d = (4,4,4,4)
                        imgs_lr = jt.nn.pad(imgs_lr, p2d, mode='reflect')
                    ...
                    imgs_lr = jt.clamp(imgs_lr, 0, 1)
                    imgs_dn = jt.clamp(imgs_dn, 0, 1)
        ...
        self.eval_psnr.sync()
        self.eval_psnr_lr.sync()
        self.eval_psnr_dn.sync()
        self.eval_ssim_lr.sync()
        self.eval_ssim_dn.sync()

        # 超过最好记录才保存
        if self.eval_psnr_dn.avg >= self.best_psnr and epoch > 0 and jt.rank == 0:
            self.best_psnr = self.eval_psnr_dn.avg
            log(f"Best PSNR is {self.best_psnr} now!!")
            model_dict = self.net.state_dict()
            jt.save(model_dict, f'./checkpoints/{self.model_name}_best_model.pth')
    ...
    def predict(self, raw, name='ds'):
        ...
        img_lr = jt.array(img_lr).float32()
        with jt.no_grad():
            ...
            img_dn = img_dn
            img_dn = img_dn[0].numpy()
        np.save(f'{name}.npy', img_dn)
    
    def preprocess(self, data, mode='train', preprocess=True):
        # 由于crops的存在，Dataloader会把数据变成5维，需要view回4维
        imgs_hr = tensor_dim5to4(data['hr']).float32()
        imgs_lr = tensor_dim5to4(data['lr']).float32()
        # self.use_gpu = True
        ...
        if self.use_gpu and mode=='train' and preprocess:
            ...
            if self.args['dst_train']['dataset'] == 'Mix_Dataset':
                data['ratio'] = data['ratio'].view(-1).float32()
                aug_r, aug_g, aug_b = get_aug_param_torch(data, b=b, command=self.dst['command'])
                aug_wbs = jt.stack((aug_r, aug_g, aug_b, aug_g), dim=1)
                data['rgb_gain'] = jt.ones(b) * (aug_g + 1)
                for i in range(b):
                ...
                for i in range(b):
                    ...
                    for key in noise_param:
                        if not isinstance(noise_param[key], jt.Var):
                            noise_param[key] = jt.array(np.array(noise_param[key], np.float32))
        ...
        ratio = data['ratio'].float32()
        ratio = ratio.view(-1,1,1,1)
        if 'rgb_gain' in data:
            data['rgb_gain'] = data['rgb_gain'].float32().view_as(ratio)
